...
describe('getHTMLProps', () => {
    ...
    it('should remove props with more than one function as parameter', () => {
        ...
        expect(getHTMLProps(props, removeHref, removeOnClick))
            .toEqual({});
    });
});
...
describe('combineModifiers', () => {
    ...
    it('should return combined Object when get*ModifiersFunctions are passed', () => {
        ...
        const combined = combineModifiers(props, getTestAnyModifiers, getTestSomeModifiers, getFailModifiers);
        expect(combined)
            .toEqual({ 'test-any': true, 'test-some': true }​​​​​);
    });
})
